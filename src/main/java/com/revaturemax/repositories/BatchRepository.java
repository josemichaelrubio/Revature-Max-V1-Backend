...
public interface BatchRepository extends JpaRepository<Batch, Long> {
        ...
        @Query(value="select eb.batch_id, eq.quiz_id, eq.score from employee_batch eb inner join employee_quiz eq on eb.employee_id=eq.employee_id where eb.batch_id = ?1 order by eq.quiz_id", nativeQuery = true)
